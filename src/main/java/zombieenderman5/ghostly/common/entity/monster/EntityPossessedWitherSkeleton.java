...
public class EntityPossessedWitherSkeleton extends AbstractSkeleton implements IRangedAttackMob, IPossessed {
	...
	private static final DataParameter<Integer> EYE_TYPE = EntityDataManager.<Integer>createKey(EntityPossessedWitherSkeleton.class, DataSerializers.VARINT);
	private static int oldEyeType = -1;
	private static int newEyeType = -1;
	private static boolean multicolorEyes = false;
	
	public EntityPossessedWitherSkeleton(World worldIn)
    {
        super(worldIn);
	...
	public void onLivingUpdate() {
		...
		if (this.getEyeType() > 3 && !GhostlyConfig.AESTHETICS.multicolorPossessedWitherSkeletonEyes && newEyeType == -1) {
			
			oldEyeType = this.getEyeType();
			this.setEyeType(this.rand.nextInt(4));
			newEyeType = this.getEyeType();
			
		} else if (this.getEyeType() > 3 && !GhostlyConfig.AESTHETICS.multicolorPossessedWitherSkeletonEyes && newEyeType != -1) {
			
			this.setEyeType(newEyeType);
			
		}
		
		if (this.getEyeType() <= 3 && multicolorEyes && GhostlyConfig.AESTHETICS.multicolorPossessedWitherSkeletonEyes && oldEyeType != -1) {
			
			this.setEyeType(oldEyeType);
			
		}
		
		if (!GhostlyConfig.MOBS.possessedWitherSkeletons) this.setDead();
    ...
    public static void registerFixesPossessedWitherSkeleton(DataFixer fixer)
    {
        EntityLiving.registerFixesMob(fixer, EntityPossessedWitherSkeleton.class);
    }
    ...
    public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, @Nullable IEntityLivingData livingdata)
    {
        ...
        if (GhostlyConfig.AESTHETICS.multicolorPossessedWitherSkeletonEyes && GhostlyConfig.AESTHETICS.multicolorPossessedWitherSkeletonEyesChance != 0 && this.rand.nextInt(GhostlyConfig.AESTHETICS.multicolorPossessedWitherSkeletonEyesChance) == 0) {
